...
Two declarations of the same name, same kind, within the scope chain that is reachable from the cursor's file (via the dep graph, include chain, and forward-call walk the system already computes) are **the same symbol**. This applies whether the redeclarations sit in the same file, in sibling branches of an `if/else`, in a parent and a do-called child, or anywhere else in the reachable chain.
...
### Hover — first definition

Hover shows the first definition, ordered by preorder index. This is the simplest stable answer. If the primary symbol has `additional_definitions`, the hover content gets a single trailing line of the form:

> Redefined at lines 12, 17, 23 — see all references

This line is optional in a first pass; pure first-def hover is acceptable for v1. Showing the *last* active-at-cursor definition would require value-flow analysis the LSP does not have, and reintroduces the per-site identity splitting this design is retiring.
...
- Show first def. Optionally append a "Redefined at lines N, M — see all references" footer when `additional_definitions` is non-empty. Ship the plain version first; add the footer behind a follow-up if we want.
...
- Hover: first def (with optional redefinition footer).
...
- **Hover.** File with three `local fruit` declarations: hover shows the first, and (if the footer is implemented) mentions the other two.
- **Forward-reference warnings unchanged.** Regression test: existing warning-on/off cases still behave as before.

## Open questions to resolve during implementation

- When the reachable chain is *asymmetric* (file A includes B, B does not include A), is a redeclaration in A the same identity as one in B from B's perspective? Rule 1 says yes for a cursor in A; the B-side question is whether find-references from within B reaches "upward" into A. The current dep-graph `callee_to_callers` + `caller_to_callees` plumbing should handle this, but we need an explicit test.
- Whether the hover redefinition footer should show file-qualified locations when redeclarations span files, or only for same-file redeclarations. Suggested default: same-file redeclarations only, to keep the hover compact.
