...
- [x] 1. Add disposable infrastructure to core components
  - [x] 1.1 Add `dispose()` to `DocumentDebounceManager`
  ...
  - [x] 1.4 Write property test for document close generation safety
    - **Property 14: Closed documents are not reinserted**
    - **Validates: Requirements 16.1, 16.2**

  - [x] 1.5 Add `dispose()` to `ScopeResolver` and `ForwardScopeResolver`
...
- [x] 3. Enhance shutdown handler and pending revalidations cleanup
  - [x] 3.1 Update `create_shutdown_handler` in `server-handlers.ts`
  ...
  - [x] 3.2 Write unit tests for shutdown handler
  ...
  - [x] 3.3 Add pending revalidations cleanup after callback completion
  ...
  - [x] 3.4 Write unit tests for pending revalidations cleanup
...
- [x] 4. Move parse into debounce callback and add request freshness
  - [x] 4.1 Refactor `validate_text_document` in `server-factory.ts`
  ...
  - [x] 4.2 Write property test for debounce coalescing
    - **Property 3: Debounce coalesces rapid changes into single callback**
    - **Validates: Requirements 2.2, 3.2**

  - [x] 4.3 Add `wait_for_debounce` calls in request handlers
  ...
  - [x] 4.4 Write unit tests for debounced parse pipeline and request freshness
...
- [x] 6. Stable handler registration with mutable deps container
  - [x] 6.1 Refactor handler registration in `server-factory.ts`
  ...
  - [x] 6.2 Write property test for mutable deps visibility
    - **Property 4: Mutable deps container visible to all handlers**
    - **Validates: Requirements 4.2, 4.3, 14.1, 14.2, 14.3**

- [x] 7. CancellationToken checking in providers and resolvers
  - [x] 7.1 Add cancellation checks in hover, definition, and references providers
  ...
  - [x] 7.2 Add cancellation checks in ScopeResolver and ForwardScopeResolver
  ...
  - [x] 7.3 Add cancellation checks in references workspace scan
    - Check cancellation periodically when scanning workspace-indexed files
    - _Requirements: 13.3_

  - [x] 7.4 Write property and unit tests for cancellation
...
- [x] 9. Token position index
  - [x] 9.1 Add `token_line_index` to `DocumentState` and build during parse
  ...
  - [x] 9.2 Add `get_token_at_position` helper method
    - Implement line-bucketed lookup with correct multi-line boundary checks
    - _Requirements: 6.1, 6.2, 12.2_

  - [x] 9.3 Write property test for token position index
    - **Property 6: Token position index matches linear scan**
    - **Validates: Requirements 6.1, 12.1, 12.2**

- [x] 10. Gated debug logging
  - [x] 10.1 Add `debug` field to `StataLSPConfig` and `DEFAULT_SETTINGS`
    - Add `debug?: boolean` to config type, default to `false`
    - _Requirements: 8.1_

  - [x] 10.2 Gate logging calls in `validate_text_document` and revalidation scheduling
  ...
  - [x] 10.3 Write property and unit tests for debug logging
    - **Property 9: Debug logging gated by config**
    - **Validates: Requirements 8.2, 8.3**
    - Unit test that debug=true emits logs (Req 8.4)

- [x] 11. Context-aware completion isIncomplete and scope resolver content source
  - [x] 11.1 Update completion handler to return context-aware `isIncomplete`
  ...
  - [x] 11.2 Write property test for isIncomplete
    - **Property 10: isIncomplete reflects macro context**
    - **Validates: Requirements 9.1, 9.2**

  - [x] 11.3 Update scope resolver content provider to prefer TextDocuments
  ...
  - [x] 11.4 Write property test for scope resolver content source
