...
- [ ] 1. Add the shared parseParenthesizedGroup method
  - [ ] 1.1 Create the `parseParenthesizedGroup` private method in `src/parser/index.ts`
  ...
  - [ ] 1.2 Write property test for nested parenthesis depth tracking
    - **Property 1: Nested Parenthesis Depth Tracking**
    - **Validates: Requirements 1.3, 4.2**

  - [ ] 1.3 Write property test for word token spacing preservation
    - **Property 2: Word Token Spacing Preservation**
    - **Validates: Requirements 1.4**

- [ ] 2. Refactor parseCommand to use shared method
  - [ ] 2.1 Replace inline LPAREN handling in `parseCommand` with call to `parseParenthesizedGroup`
  ...
  - [ ] 2.2 Write property test for parsing consistency
    - **Property 3: Parsing Consistency After Refactoring**
    - **Validates: Requirements 2.3, 3.3, 4.1, 4.3, 5.3, 5.4**

- [ ] 3. Refactor parseCommandBody to use shared method
  - [ ] 3.1 Replace inline LPAREN handling in `parseCommandBody` with call to `parseParenthesizedGroup`
    - Locate the LPAREN handling block (around lines 1145-1185)
    - Replace with: `const paren_node = this.parseParenthesizedGroup(); if (paren_node) { varlist.push(paren_node); }`
    - _Requirements: 3.1, 3.2, 3.3_

  - [ ] 3.2 Write property test for direct vs frame-prefixed equivalence
