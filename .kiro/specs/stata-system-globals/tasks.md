...
- [x] 1. Add system globals constant and helper function
  - [x] 1.1 Add `STATA_SYSTEM_GLOBALS` constant set to `src/analyzer/index.ts`
  ...
  - [x] 1.2 Add `is_system_global` helper method to `SemanticAnalyzer` class
    - Add private method that checks if name is in STATA_SYSTEM_GLOBALS
    - Place near `is_positional_argument` for logical grouping
    - _Requirements: 4.3_

- [x] 2. Integrate system global check into is_macro_defined
  - [x] 2.1 Modify `is_macro_defined` to check system globals as fallback
...
- [x] 4. Add property-based tests
  - [x] 4.1 Write property test for system globals not flagged
  ...
  - [x] 4.2 Write property test for case-sensitive matching
  ...
  - [x] 4.3 Write property test for non-system globals still flagged
  ...
  - [x] 4.4 Write property test for is_system_global function
...
- [x] 5. Add unit tests
  - [x] 5.1 Write unit tests for all system globals
