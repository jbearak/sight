...
- [x] 1. Enhance IndentationDiagnosticAnalyzer with depth-based analysis
  - [x] 1.1 Add `compute_expected_depths()` method to compute expected indentation depth for each line using AST traversal
  ...
  - [x] 1.2 Add `should_skip_unnecessary_check()` method to identify excluded lines
  ...
  - [x] 1.3 Add `find_unnecessary_indentation_issues()` method for over-indentation detection
  ...
  - [x] 1.4 Update `analyze()` to call new methods and combine diagnostics
...
- [x] 1.5 Write property test for top-level unnecessary indentation detection
  - **Property 1: Top-level unnecessary indentation detection**
  - **Validates: Requirements 1.1, 2.1**

- [x] 1.6 Write property test for correct indentation producing no diagnostic
  - **Property 2: Correct indentation produces no unnecessary diagnostic**
  - **Validates: Requirements 1.2**

- [x] 1.7 Write property test for excluded lines
  - **Property 5: Excluded lines produce no unnecessary diagnostic**
  - **Validates: Requirements 2.3, 2.4**

- [x] 2. Checkpoint - Verify diagnostic detection
  - Ensure all diagnostic tests pass, ask the user if questions arise.

- [x] 3. Enhance formatter for mixed indentation normalization
  - [x] 3.1 Review `TokenReconstructor.reconstruct()` to ensure mixed indentation is fully replaced
  ...
  - [x] 3.2 Verify `make_indent()` generates correct indentation for both styles
    - Spaces mode: `' '.repeat(level * indent_size)`
    - Tabs mode: `'\t'.repeat(level)`
    - _Requirements: 3.2, 3.3_

- [x] 3.3 Write property test for formatter indentation normalization
  - **Property 6: Formatter normalizes indentation to configured style**
  - **Validates: Requirements 3.1, 3.2, 3.3**

- [x] 3.4 Write property test for content preservation
  - **Property 7: Formatter preserves non-whitespace content**
  - **Validates: Requirements 3.4**

- [x] 4. Checkpoint - Verify formatter behavior
  - Ensure all formatter tests pass, ask the user if questions arise.

- [x] 5. Integration and round-trip verification
  - [x] 5.1 Add integration test verifying diagnostic-formatter consistency
...
- [x] 5.2 Write property test for round-trip elimination of diagnostics
