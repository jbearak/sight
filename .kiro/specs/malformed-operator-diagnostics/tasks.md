...
- [ ] 4. Implement the OperatorSequenceAnalyzer
  ...
  - [-] 4.2 Write property test: suggestible pair detection (Property 1)
  ...
  - [ ] 4.3 Write property test: invalid pair detection (Property 2)
  ...
  - [ ] 4.4 Write property test: no false positives for allowed adjacencies (Property 4)
  ...
  - [ ] 4.5 Write property test: config severity override for suggestible (Property 8)
  ...
  - [ ] 4.6 Write property test: config severity override for invalid (Property 9)
...
- [ ] 5. Continuation and statement boundary handling
  - [ ] 5.1 Ensure the adjacency scanner in `OperatorSequenceAnalyzer` correctly handles `///` continuation tokens between operators (treats them as trivia, so pairs spanning continuations are detected)
    - _Requirements: 6.1_
  - [ ] 5.2 Write property test: continuation-spanning detection (Property 5)
  ...
  - [ ] 5.3 Write property test: statement terminator boundary (Property 6)
...
- [ ] 6. Embedded context filtering and suppression
  - [ ] 6.1 Integrate `OperatorSequenceAnalyzer` into `DiagnosticsProvider.get_diagnostics()` in `src/providers/diagnostics.ts`
  ...
  - [ ] 6.2 Write property test: embedded context suppression (Property 3)
  ...
  - [ ] 6.3 Write property test: directive suppression (Property 7)
...
- [ ] 7. VS Code extension settings and documentation
  - [ ] 7.1 Add `sight.diagnostics.severity.malformedOperator` and `sight.diagnostics.severity.invalidOperatorSequence` entries to `client/package.json` `contributes.configuration.properties`, following the existing severity setting pattern
    - _Requirements: 8.1, 8.2, 8.6, 8.7_
  - [ ] 7.2 Add documentation rows to `README.md` diagnostics settings table for the two new severity settings
    - _Requirements: 8.1, 8.2_

- [ ] 8. Unit tests for exact messages and edge cases
  - [ ] 8.1 Write unit tests in `tests/unit/operator-sequence-diagnostics.test.ts`
...
- [ ] 9. Integration tests
  - [ ] 9.1 Write integration tests in `tests/integration/operator-sequence-diagnostics.test.ts`
