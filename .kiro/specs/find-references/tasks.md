...
- [x] 1. Create ReferencesProvider core implementation
  - [x] 1.1 Create `src/providers/references.ts` with ReferencesProvider class
  ...
  - [x] 1.2 Implement symbol identification at cursor position
  ...
  - [x] 1.3 Write property test for symbol identification
    - **Property 2: Empty Result for Invalid Position**
    - **Validates: Requirements 1.7**

- [x] 2. Implement token scanning for references
  - [x] 2.1 Implement `scan_tokens_for_references()` function
  ...
  - [x] 2.2 Implement name extraction from tokens
  ...
  - [x] 2.3 Write property test for case-sensitive matching
    - **Property 5: Case-Sensitive Matching**
    - **Validates: Requirements 4.5**

- [x] 3. Implement workspace-wide search
  - [x] 3.1 Extend WorkspaceIndexer with `get_indexed_files()` method
    - Return Map of URI to tokens and context ranges
    - Expose indexed file data for reference search
    - _Requirements: 3.1, 3.2_
  
  - [x] 3.2 Implement search across indexed files
  ...
  - [x] 3.3 Write property test for workspace coverage
    - **Property 7: Workspace Coverage**
    - **Validates: Requirements 3.1, 3.2**
  
  - [x] 3.4 Write property test for fresh content
...
- [x] 7. Implement embedded language context handling
  ...
  - [x] 7.3 Write property test for macros crossing contexts
    - **Property 9: Macros Cross Embedded Contexts**
    - **Validates: Requirements 8.1, 8.3**
  
  - [x] 7.4 Write property test for non-macros excluded
    - **Property 10: Non-Macros Excluded from Embedded Contexts**
    - **Validates: Requirements 8.2, 8.4**

- [x] 8. Checkpoint - Ensure all provider logic works
  - Ensure all tests pass, ask the user if questions arise.

- [x] 9. Wire up LSP server integration
  - [x] 9.1 Register referencesProvider capability in server initialization
    - Add `referencesProvider: true` to InitializeResult capabilities
    - _Requirements: 9.1_
  
  - [x] 9.2 Create references handler in server-handlers.ts
    - Create `create_references_handler()` factory function
    - Wire up to HandlerDependencies
    - _Requirements: 9.2_
  
  - [x] 9.3 Register handler in server-factory.ts
    - Add `connection.onReferences()` handler
    - Pass dependencies to handler
    - _Requirements: 9.2_

- [x] 10. Write comprehensive property test for symbol search
  - [x] 10.1 Write property test for symbol identification and search
